"use client";
import { ChangeEvent, FC, useState } from "react";
import { todoType } from "@/types/todoType";

interface Props {
  todo: todoType;
  changeTodoText: (id: number, text: string) => Promise<void>;
  toggleIsTodoDone: (id: number) => Promise<void>;
  deleteTodoItem: (id: number) => Promise<void>;
}

const Todo: FC<Props> = ({
  todo,
  changeTodoText,
  toggleIsTodoDone,
  deleteTodoItem,
}) => {
  // State for handling editing mode
  const [editing, setEditing] = useState(false);

  // State for handling text input
  const [text, setText] = useState(todo.text);

  // State for handling "done" status
  const [isDone, setIsDone] = useState(todo.done);

  // State for handling loading states
  const [isLoading, setIsLoading] = useState(false);

  // Event handler for text input change
  const handleTextChange = (e: ChangeEvent<HTMLInputElement>) => {
    setText(e.target.value);
  };

  // Event handler for toggling "done" status
  const handleIsDone = async () => {
    if (isLoading) return;

    setIsLoading(true);
    setIsDone((prev) => !prev);

    try {
      await toggleIsTodoDone(todo.id);
    } catch (error) {
      // Rollback on error
      setIsDone((prev) => !prev);
    } finally {
      setIsLoading(false);
    }
  };

  // Event handler for initiating the edit mode
  const handleEdit = () => {
    setEditing(true);
  };

  // Event handler for saving the edited text
  const handleSave = async () => {
    if (isLoading || !text.trim()) return;

    setIsLoading(true);

    try {
      await changeTodoText(todo.id, text);
      setEditing(false);
    } catch (error) {
      // Keep editing mode open on error
      console.error("Failed to save todo:", error);
    } finally {
      setIsLoading(false);
    }
  };

  // Event handler for canceling the edit mode
  const handleCancel = () => {
    setEditing(false);
    setText(todo.text);
  };

  // Event handler for deleting a todo item
  const handleDelete = async () => {
    if (isLoading) return;

    if (confirm("Are you sure you want to delete this todo?")) {
      setIsLoading(true);
      try {
        await deleteTodoItem(todo.id);
      } catch (error) {
        console.error("Failed to delete todo:", error);
      } finally {
        setIsLoading(false);
      }
    }
  };

  // Rendering the Todo component
  return (
    <div className={`flex items-center gap-2 p-4 border-gray-200 border-solid border rounded-lg ${isLoading ? "opacity-50 pointer-events-none" : ""}`}>
      {/* Checkbox for marking the todo as done */}
      <input
        type="checkbox"
        className="text-blue-200 rounded-sm h-4 w-4"
        checked={isDone}
        onChange={handleIsDone}
        disabled={isLoading}
      />
      {/* Input field for todo text */}
      <input
        type="text"
        value={text}
        onChange={handleTextChange}
        readOnly={!editing}
        disabled={isLoading}
        className={`${
          todo.done ? "line-through" : ""
        } outline-none read-only:border-transparent focus:border border-gray-200 rounded px-2 py-1 w-full disabled:opacity-50`}
      />
      {/* Action buttons for editing, saving, canceling, and deleting */}
      <div className="flex gap-1 ml-auto">
        {editing ? (
          <button
            onClick={handleSave}
            disabled={isLoading || !text.trim()}
            className="bg-green-600 text-green-50 rounded px-2 w-14 py-1 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isLoading ? "..." : "Save"}
          </button>
        ) : (
          <button
            onClick={handleEdit}
            disabled={isLoading}
            className="bg-blue-400 text-blue-50 rounded w-14 px-2 py-1 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            Edit
          </button>
        )}
        {editing ? (
          <button
            onClick={handleCancel}
            disabled={isLoading}
            className="bg-red-400 w-16 text-red-50 rounded px-2 py-1 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            Close
          </button>
        ) : (
          <button
            onClick={handleDelete}
            disabled={isLoading}
            className="bg-red-400 w-16 text-red-50 rounded px-2 py-1 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isLoading ? "..." : "Delete"}
          </button>
        )}
      </div>
    </div>
  );
};

export default Todo;
